    ...
    Parameters:
        ...
        fsdp (`bool`, `str` or list of [`~trainer_utils.FSDPOption`], *optional*, defaults to `''`):
            ...
            - ``hybrid_shard``: Apply ``full_shard`` within a node, and replicate parameters across nodes.
               This results in reduced communication volume as expensive all-gathers and reduce-scatters
               are only done within a node, which can be more performant for medium-sized models.
            - ``_hybrid_shard_zero2``: Apply ``shard_grad_op`` within a node, and replicate parameters across
               nodes. This is like ``hybrid_shard``, except this may provide even higher throughput since the
               unsharded parameters are not freed after the forward pass, saving the all-gathers in the pre-backward.
    ...
    fsdp: Optional[Union[List[FSDPOption], str]] = field(
        ...
        metadata={
            "help": (
                ...
                " only). The base option should be `full_shard`, `shard_grad_op`, `hybrid_shard`, `_hybrid_shard_zero2`"
                "or `no_shard` and you can add CPU-offload to `full_shard` or `shard_grad_op` like this: "
